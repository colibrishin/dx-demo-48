...
// https://raw.githubusercontent.com/walbourn/directx-vs-templates/main/d3d12game_win32_dr/StepTimer.h
...
namespace Engine
{
    ...
    public:
        ...
        float GetElapsedSeconds() const noexcept { return TicksToSeconds(m_elapsedTicks); }

        // Get total time since the start of the program.
        uint64_t GetTotalTicks() const noexcept { return m_totalTicks; }
        float GetTotalSeconds() const noexcept { return TicksToSeconds(m_totalTicks); }
        ...
        void SetTargetElapsedSeconds(float targetElapsed) noexcept { m_targetElapsedTicks = SecondsToTicks(targetElapsed); }

        // Integer format represents time using 10,000,000 ticks per second.
        static constexpr uint64_t TicksPerSecond = 10000000;

        static constexpr float TicksToSeconds(uint64_t ticks) noexcept { return static_cast<float>(ticks) / TicksPerSecond; }
        static constexpr uint64_t SecondsToTicks(float seconds) noexcept { return seconds * TicksPerSecond; }
        ...
        static void Initialize()
        {
	        if(!m_delta_time_)
	        {
		        m_delta_time_ = std::unique_ptr<DeltaTime>(new DeltaTime());
	        }
        }
    ...
    private:
        ...
        inline static std::unique_ptr<DeltaTime> m_delta_time_ = nullptr;
